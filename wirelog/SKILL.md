...
Use `wl dashboard sync --file <path>` to version validated YAML in the API credential's project. Synced dashboards appear in the authenticated dashboard workspace, its indented project-sidebar document rail, and its title-adjacent switcher; both controls include shared dashboards and the current user's personal dashboards. Add a stable root `id`. Personal `aat_` tokens need `dashboards` scope; use `--visibility project` for current project members and for directory syncs the whole team should see. Omitted visibility preserves an existing dashboard's access. An empty web workspace shows the starter, preview, and sync commands and links to the guide and token workflow. Sync never creates a standalone/public URL and never accepts a project ID.
